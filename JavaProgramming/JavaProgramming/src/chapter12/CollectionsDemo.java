...
public class CollectionsDemo {
    ...
    public static void setDemo(){
        ...
        var i= fruit.iterator(); // 1.way
        while (i.hasNext()){
            System.out.println(i.next());
        }

        for (String item: fruit){   // 2. way
            System.out.println(item);
        }

        fruit.forEach(x -> System.out.println(x)); //3. way -Lambada expression
    }
    ...
    public static void mapDemo(){
        ...
        for(var entry: fruitCalories.entrySet()){ //1.st way
            System.out.println(entry.getValue());
        }

        fruitCalories.forEach(
                (k,v) -> System.out.println("Fruit:" + k + " Calories" + k) //2nd way
    );
